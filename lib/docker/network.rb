...
class Docker::Network
  ...
  def reload
    network_json = @connection.get("/networks/#{Docker::Util.escape(@id)}")
  ...
  class << self
    ...
    def get(id, opts = {}, conn = Docker.connection)
      network_json = conn.get("/networks/#{Docker::Util.escape(id)}", opts)
      hash = Docker::Util.parse_json(network_json) || {}
      new(conn, hash)
    end
    ...
    def remove(id, opts = {}, conn = Docker.connection)
      conn.delete("/networks/#{Docker::Util.escape(id)}", opts)
      nil
    end
    alias_method :delete, :remove
